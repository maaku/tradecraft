...
public:
    ...
    // a block within a the auxiliary chain (presumably a bitcoin block).  Once
    // a solution is found for the auxiliary proof-of-work, the miner then finds
    // a satisfactory proof-of-work solution under the old rules for effectively
    ...
    // FIXME: This field is going to be used to store the difficulty
    //        adjustment filter prediction for the prior block.
    ...
    //
    //   commitment = H(H(block_header) || H(m_secret_lo || m_secret_hi))
    //
    // Where H() is the fast Merkle tree hash.
    //
...
public:
    ...
    int64_t GetFilteredTime() const
    {
        ...
        // as a 32.32 fixed point number, however, with the low 8 bits clear.
