...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW)
{
    ...
    if (!block.m_aux_pow.IsNull()) {
        ...
        if (aux_pow.m_aux_path.size() > MAX_AUX_POW_PATH_LENGTH) {
            return state.DoS(100, false, REJECT_INVALID, "auxpow-merkle-path-size", false, "auxiliary proof-of-work Merkle map path is too long");
        }

        size_t nbits = 0;
        for (size_t idx = 0; idx < aux_pow.m_aux_path.size(); ++idx) {
            ++nbits;
            nbits += aux_pow.m_aux_path[idx].first;
        }
        if (nbits >= 256) {
            return state.DoS(100, false, REJECT_INVALID, "auxpow-merkle-path-bits", false, "auxiliary proof-of-work Merkle map path is greater than 256 bits");
        }
    }
