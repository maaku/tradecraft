...
std::pair<int64_t, int64_t> GetFilteredAdjustmentFactorAux(const CBlockIndex* pindexLast, const Consensus::Params& params)
{
...
unsigned int CalculateNextWorkRequired(const CBlockIndex* pindexLast, const Consensus::Params& params, bool protocol_cleanup)
{
    ...
    if (protocol_cleanup) {
        bnPowLimit = UintToArith256(k_min_pow_limit);
    }
    arith_uint256 bnNew;
...
{
    arith_uint256 bnPowLimit = UintToArith256(params.aux_pow_limit);
    ...
    adjustment_factor = GetFilteredAdjustmentFactorAux(pindexLast, params);
    ...
    // Retarget
    arith_uint256 bnNew;
    bnNew.SetCompact(pindexLast->m_aux_pow.m_commit_bits);
    arith_uint320 bnTmp(bnNew);
    bnTmp *= arith_uint320(static_cast<uint64_t>(adjustment_factor.second));
    bnTmp /= arith_uint320(static_cast<uint64_t>(adjustment_factor.first));
    assert(bnTmp.TruncateTo256(bnNew));
...
bool CheckAuxiliaryProofOfWork(const CBlockHeader& block)
{
    ...
    if ((256 - bnTarget.bits()) < bias) {
        return false;
    }
    bnTarget <<= bias;

    // Check auxiliary proof-of-work (1st stage)
    if (UintToArith256(aux_hash.first) > bnTarget) {
        return false;
    }

    // Calculate target for 2nd stage.
    bnTarget = UintToArith256(uint256S("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"));
    bnTarget >>= bias;

    // Check auxiliary proof-of-work (2nd stage)
    if (UintToArith256(aux_hash.second) > bnTarget) {
        return false;
    }
